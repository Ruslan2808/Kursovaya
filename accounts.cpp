...
void showAccounts(Accounts* mas_of_accounts, int number_of_accounts, int r)
{
	...
	for (int i = 0; i < number_of_accounts; i++) {
		...
		cout << "Логин: " << mas_of_accounts[i].getLogin() << endl;
		cout << "Пароль: " << mas_of_accounts[i].getPassword() << endl;
		cout << "Состояние: ";
		if (mas_of_accounts[i].getAcess() == 0) {
			cout << "не подтвержден" << endl;
		}
		else cout << "подтвержден" << endl;
		Sleep(500);
	}
...
void updateAccounts(Accounts*& mas_of_accounts, int number_of_accounts) {
	...
	if (q == 0) {
		cout << "Аккаунтов с логиноЫЫм " << login << " нет" << endl;
	}
	else {
